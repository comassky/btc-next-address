...
public class AddressService {
    ...
    @Inject BlockchainChecker blockchainChecker;
    @Inject AddressCacheManager cacheManager;
    ...
    private String generateQrCodeSvgBase64(String text) {
        try {
            ...
            sb.append("<svg xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\" viewBox=\"0 0 ")
              ...
              .append("<path fill=\"#ffffff\" d=\"M0 0h").append(width).append("v").append(height).append("H0z\"/>")
              .append("<path stroke=\"#000000\" d=\"");
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    if (bitMatrix.get(x, y)) sb.append("M").append(x).append(" ").append(y).append("h1v1h-1z ");
                }
            }
            sb.append("\"/></svg>");
            return "data:image/svg+xml;base64," + Base64.getEncoder().encodeToString(sb.toString().getBytes());
        } catch (WriterException e) { return ""; }
    }
